...
def add_truck(truck_data):
    ...
    """
    trucks_table = current_app.config.get('DYNAMODB_TRUCKS_TABLE')
    logging.info(f"[add_truck] Called with data: {truck_data}. Target table: {trucks_table}")
    try:
        ...
        if missing_fields:
            logging.warning(f"[add_truck] Missing required fields: {missing_fields}")
            return {'error': f'Missing required fields: {missing_fields}'}, 400

        # Validate types
        if not isinstance(truck_data['truck_name'], str):
            logging.warning(f"[add_truck] Invalid truck_name type: {type(truck_data['truck_name'])}")
            return {'error': 'Truck name must be a string'}, 400
        if not isinstance(truck_data['truck_model'], str):
            logging.warning(f"[add_truck] Invalid truck_model type: {type(truck_data['truck_model'])}")
            return {'error': 'Truck model must be a string'}, 400
        if not isinstance(truck_data['manufacture_year'], int):
            logging.warning(f"[add_truck] Invalid manufacture_year type: {type(truck_data['manufacture_year'])}")
        ...
        if 'truck_id' not in truck_data:
            ...
            logging.info(f"[add_truck] Generated new truck ID: {truck_data['truck_id']}")
        ...
        for key, value in truck_data.items():
            if isinstance(value, float):
                ...
                logging.info(f"[add_truck] Converted float to int for {key}: {value} -> {truck_data[key]}")
        
        # Store truck in DynamoDB
        logging.info(f"[add_truck] Attempting to store truck in DynamoDB: {truck_data}")
        response = dynamodb_put_item(trucks_table, truck_data)
        logging.info(f"[add_truck] Successfully added truck. DynamoDB response: {response}")
        return truck_data
    except Exception as e:
...
def update_truck(truck_id, truck_data):
    ...
    """
    trucks_table = current_app.config.get('DYNAMODB_TRUCKS_TABLE')
    logging.info(f"[update_truck] Updating truck_id={truck_id} in table {trucks_table}")
    logging.info(f"[update_truck] Update data received: {truck_data}")
    
    # Debug: Log the incoming update payload
    logging.debug(f"[update_truck] truck_id={truck_id}, incoming payload: {truck_data}")
    ...
    if 'truck_name' in truck_data and not isinstance(truck_data['truck_name'], str):
        logging.warning(f"[update_truck] Invalid truck_name type: {type(truck_data['truck_name'])}")
        return {'error': 'Truck name must be a string'}, 400
    if 'truck_model' in truck_data and not isinstance(truck_data['truck_model'], str):
        logging.warning(f"[update_truck] Invalid truck_model type: {type(truck_data['truck_model'])}")
        return {'error': 'Truck model must be a string'}, 400
    if 'manufacture_year' in truck_data and not isinstance(truck_data['manufacture_year'], int):
        logging.warning(f"[update_truck] Invalid manufacture_year type: {type(truck_data['manufacture_year'])}")
        return {'error': 'Manufacture year must be an integer'}, 400
    ...
    for key, value in truck_data.items():
        if isinstance(value, float):
            truck_data[key] = int(value)
            logging.info(f"[update_truck] Converted float to int for {key}: {value} -> {truck_data[key]}")
    ...
    try:
        ...
        logging.info(f"[update_truck] Successfully updated truck. Response: {response}")
...
def delete_truck(truck_id):
    ...
    """
    trucks_table = current_app.config.get('DYNAMODB_TRUCKS_TABLE')
    logging.info(f"[delete_truck] Deleting truck_id={truck_id} from table {trucks_table}")
    ...
    try:
        ...
        logging.info(f"[delete_truck] Successfully deleted truck. Response: {response}")

...
def serve_image(filename):
    ...
    image_folder = os.path.join(base_dir, 'app/static/output')
...
def get_truck_stats():
    ...
    active_count = sum(1 for truck in trucks if truck.get('status') == 'active')
...
def get_route_stats():
    ...
    scheduled_count = sum(1 for route in routes if route.get('status') == 'scheduled')
    inprogress_count = sum(1 for route in routes if route.get('status') == 'in_progress')
    completed_count = sum(1 for route in routes if route.get('status') == 'completed')
...
def get_recent_alerts():
    ...
    recent_alerts = sorted(alerts, key=lambda x: x.get('created_at', 0), reverse=True)[:5]
